...
class Board:
    ...
    def strategyOne(self, x2, y2, maskFrameCheck, boolLayerExt=False):
        ...
        if idPoint in setF:
            ...
            ##! checking if it improves the performance ...
            # if maxSim == 0:
            #     return setF[idxCheckBoundaries]
            # else:
            #     actions = setF[*np.where(np.logical_and(similarityVec == maxSim, idxCheckBoundaries))]
...
class PipeMania(Problem):
    ...
    def actions(self, state: PipeManiaState):
        ...
        ##! Including Heuristic for the non-deterministic actions:
        ##! MRV heuristic: the pieces that have less options are put first
        ##! Higher degree heuristic: the pieces that have more adjacent pieces are put first
    ...
    def goal_test(self, state: PipeManiaState):
        ...
        # ##! check if all pices are in the right place
        # tableArr = np.array([[state.board.lookUpFunc(x) for x in row] for row in state.board.table], dtype=npType)
        # mSize = tableArr.shape[0]
...
if __name__ == "__main__":
    ...
    # ## depth 4000 + these 4 changes
    # # node.state = problem.result(node.state, [9,4,"BC"])
    # # node.state = problem.result(node.state, [9,5,"VC"])
    # # node.state = problem.result(node.state, [9,6,"FB"])
    # # node.state = problem.result(node.state, [10,6,"BC"])
